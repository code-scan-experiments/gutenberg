...
-   `Modal`: Use the `title` as the dialog's `aria-label` when the header is hidden instead of referencing an unrendered heading, and warn in development when no accessible name is provided.
